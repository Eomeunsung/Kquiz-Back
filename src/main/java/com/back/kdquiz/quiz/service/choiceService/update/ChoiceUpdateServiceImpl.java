...
@AllArgsConstructor
@Slf4j
public class ChoiceUpdateServiceImpl implements ChoiceUpdateService{

    private final ChoiceRepository choiceRepository;

    @Transactional
    @Override
    public ResponseEntity choiceUpdateResponse(ChoiceUpdateDto choiceUpdateDto) {
    ...
    @Transactional
    @Override
    public Boolean choiceUpdateDto(ChoiceUpdateDto choiceUpdateDto) {
        Optional<Choice> choiceOptional = choiceRepository.findById(choiceUpdateDto.getId());
        if(choiceOptional.isEmpty()){
            throw new ChoiceNotFoundException();
        }
